...
void Process::defense(double motorPower)
{
    ...
    if ((lineDetection.linepresent && lineDetection.Chord() > 0.8) || lineDetection.outOfBounds)
    {
        motor.Move(lineAngle, motorPower, goalie.defenseOrientation(getGoal(), motor.getOrientation(), motor.initialOrientation));
    }
    else
    {
        cam.CamCalc();
        ...
        {
            smoothMove(goalie.defenseCalc(cam.ball, getGoal()), lineAngle, motorPower, goalie.defenseOrientation(getGoal(), motor.getOrientation(), motor.initialOrientation));
        }
    }
}
